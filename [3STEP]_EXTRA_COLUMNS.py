...
# COIN_RAW 폴더 경로
coin_raw_dir = r'/Users/hyeonwhan.ko/Desktop/BIGDATA/COINRAW'
...
def process_add_symbols():
    ...
    for file_name in os.listdir(coin_raw_dir):
        symbol = os.path.splitext(file_name)[0]
        file_path = os.path.join(coin_raw_dir, file_name)
    ...
    # 추가할 열들
    additional_columns2 = [
        '20MA_margin', '50MA_margin', '60MA_margin', '120MA_margin'
    ]

    list_btc = ['BTC.csv']

    # 각 symbol의 CSV 파일에서 데이터 읽어오고 열 추가 및 데이터 입력
    for file_name in list_btc:
        symbol = file_name
        file_path = os.path.join(coin_raw_dir, file_name)
        ...
        # 추가 열 생성 및 데이터 입력
        for column in additional_columns2:
            df[column] = 0  # 초기화

        # close 값이 0인 경우 모든 데이터를 0으로 입력
        if df['close'].eq(0).all():
            df.loc[:, additional_columns2] = 0
        else:
            # 각 열의 데이터 계산 및 입력
            df['timestamp'] = pd.to_datetime(df['timestamp'])  # timestamp 열을 날짜 형식으로 변환
            for index, row in df.iterrows():
